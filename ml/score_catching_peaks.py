...
def _compute_metric_from_pts(
    ...
    *,
    ...
    def peak_avg_exceeds_surrounding(
        ...
        peak_avg2 = peak_sum / float(peak_slice.size)
        ...
        if not np.isfinite(peak_avg2) or not np.isfinite(surround_avg) or surround_avg <= 0:
            return False
        if not np.isfinite(factor) or factor <= 0:
            factor = 2.0
        return float(peak_avg2) > float(factor) * float(surround_avg)
...
def main():
    ...
    out = {
        ...
        "mid_outlier_above_mean_mult": float(MID_OUTLIER_ABOVE_MEAN_MULT)
        if np.isfinite(MID_OUTLIER_ABOVE_MEAN_MULT) and MID_OUTLIER_ABOVE_MEAN_MULT > 0
        else None,
